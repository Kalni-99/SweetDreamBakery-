...
<table class="table table-responsive table-hover" style="border: 1px dashed #8c8b8b;
  ...
  $count = 0;
  $id = 0;
  $query = mysql_query("SELECT * FROM tblcnp WHERE status = 'Available' order by id desc") or die(mysql_error());
  ...
  ?>
      <tr style="border: 1px dashed #8c8b8b; cursor:pointer;">
        ...
        <td style="border: 1px dashed #8c8b8b;">
          <center><img src="<?php echo $row['image'] ?>" width="120px;" class="img-responsive img-rounded wow fadeInDown"></center>
        </td>
        <td style="border: 1px dashed #8c8b8b;">
            ...
            <form class="form-horizontal" enctype="multipart/form-data" method="post" action="">
              <div class="modal-body">
                ...
                <div class="row" id="datepickup">
                  ...
                  <div class="col-lg-10">
                    <input type="date" name="datep" value="<?php echo date('Y-m-d'); ?>" class="form-control" />
                  </div>
                </div>
  ...
  <?php }
    ...
    echo '<strong style="color:red">No availables data in server</strong>';
  } ?>
</table>
...
if (isset($_POST['savechanges'])) {
  ...
  $otype = $_POST['otype'];
  $datep = $_POST['datep'];
  $query = mysql_query("SELECT * FROM tblorders WHERE cname = '$name'") or die(mysql_error());
  if (mysql_num_rows($query) == 5) {
    echo '<script>alert("You reach maximum order of 5");
								window.loaction.href="availableframe.php";</script>';
  } else {

    $sql = mysql_query("INSERT INTO tblorders VALUES(NULL,'$name','$address','$contact','$id','$qty','new','$otype','$datep',now())") or die(mysql_error());
    if ($sql == true) {
